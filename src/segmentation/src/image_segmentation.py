...
def threshold_segment_naive(gray_img, lower_thresh, upper_thresh):
    ...
    out = np.ones(gray_img.shape)
    for x in range(out.shape[0]):
        for y in range(out.shape[1]):
            if gray_img[x,y] < lower_thresh or gray_img[x,y] > upper_thresh:
                out[x,y] = 0
    return out
...
def edge_detect_naive(gray_img):
    ...
    hi = np.square(G_x) + np.square(G_y)
    G = np.sqrt(hi)
...
def segment_image(img): 
    ...
    binary = threshold_segment_naive(to_grayscale(img), 55, 70).astype(np.uint8)

    # perform clustering segmentation.
    # binary = cluster_segment(img, 3).astype(np.uint8)
